...
num_images = 10000
gen_batch_size = 64
eval_batch_size = 32   # 🔥 prevents OOM


# ================= PATHS =================
fake_dir = os.path.join(BASE_DIR, "is_fake_dcgan_tiny")

checkpoint_path = os.path.join(
    BASE_DIR,
    "checkpoints",
    "dcgan_tinyimagenet_generator_final.pth"
)
    ...
    for i in range(0, len(files), eval_batch_size):
        ...
        for j in range(i, min(i + eval_batch_size, len(files))):
            try:
                ...
                print(f"⚠️ Skipping corrupted image: {files[j]}")
                continue
...
# ================= OUTPUT =================
print(f"\n DCGAN TinyImageNet IS: {is_mean:.4f} ± {is_std:.4f}")